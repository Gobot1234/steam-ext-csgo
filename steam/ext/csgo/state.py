...
class GCState(ConnectionState):
    ...
    def __init__(self, client: Client, **kwargs: Any):
        ...
        self._unpatched_inventory: Callable[[Game], Coroutine[
            None, None, Inventory]] = None  # type: ignore
    ...
    async def parse_gc_message(self, msg: MsgProto[CMsgGcClient]) -> None:
        ...
        try:
            msg = (GCMsgProto(language, msg.body.payload) if utils.is_proto(
                msg.body.msgtype) else GCMsg(language, msg.body.payload))
        except Exception as exc:
            return log.error(
                f"Failed to deserialize message: {language!r}, {msg.body.payload!r}",
                exc_info=exc)
        else:
            log.debug(f"Socket has received GC message %r from the websocket.",
                      msg)
    ...
    def parse_client_goodbye(
            self,
            msg: GCMsgProto[gcsdk.ConnectionStatus] | None = None) -> None:
    ...
    @register(Language.ClientWelcome)
    async def parse_gc_client_connect(
            self, msg: GCMsgProto[gcsdk.ClientWelcome]) -> None:
    ...
    async def update_backpack(self,
                              *cso_items: base.Item,
                              is_cache_subscribe: bool = False) -> Backpack:
        await self.client.wait_until_ready()

        backpack: Backpack = self.backpack or Backpack(
            await self._unpatched_inventory(CSGO))
        ...
        for cso_item in cso_items:  # merge the two items
            ...
            if item is None:
                ...
                for attribute_name in cso_item.__annotations__:
                    setattr(item, attribute_name,
                            getattr(cso_item, attribute_name))
            ...
            if paint_seed:
                self.set("paint_seed",
                         math.floor(READ_F32(paint_seed.value_bytes)[0]))
            ...
            for i in range(1, 6):
                sticker_id = utils.get(cso_item.attribute,
                                       def_index=113 + (i * 4))
                if sticker_id:
                    sticker = Sticker(slot=i,
                                      sticker_id=READ_U32(
                                          sticker_id.value_bytes)[0])

                    for idx, attr in enumerate(attrs):
                        attribute = utils.get(item.attribute,
                                              def_index=114 + (i * 4) + idx)
                        if attribute:
                            setattr(sticker, attribute,
                                    READ_F32(attribute.value_bytes)[0])
    ...
    def handle_matchmaking_client_hello(
            self, msg: GCMsgProto[cstrike.MatchmakingClientHello]):
    ...
    def handle_client_preview_data_block_response(
            self,
            msg: GCMsgProto[cstrike.Client2GcEconPreviewDataBlockResponse]):
        # decode the wear
    ...
    def handle_item_customization_notification(
            self, msg: GCMsgProto[econ.ItemCustomizationNotification]):
    ...
    async def handle_so_create(self, msg: GCMsgProto[gcsdk.SingleObject]):
        ...
        item = utils.get(backpack, asset_id=cso_item.id)
        if item is None:
            return log.info("Received an item that isn't our inventory %r",
                            cso_item)
    ...
    async def handle_so_update_multiple(
            self, msg: GCMsgProto[gcsdk.MultipleObjects]):
    ...
    def handle_so_destroy(self, msg: GCMsgProto[gcsdk.SingleObject]):
        ...
        for attribute_name in deleted_item.__annotations__:
            setattr(item, attribute_name, getattr(deleted_item,
                                                  attribute_name))
